...
export default function Post(props) {
    ...
    useEffect(() => {
        const fetchdata=async ()=>{
            const res=await axios.get(process.env.REACT_APP_URL+`api/users/${props.post?.userId}`)
            setuser(res.data)
        }
        fetchdata()
    },[props.post?.userId])
    const handlelikes= async()=>{
       ...
       setisLiked(!isLiked)
        await axios.put(process.env.REACT_APP_URL+`api/posts/${props.post._id}/like`,{userId:curruser._id})
    ...
    const handledelete=async()=>{
        ...
        {
            const res=await axios.delete(process.env.REACT_APP_URL+`api/posts/${props.post._id}`)
