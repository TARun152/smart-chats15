...
export default function Messenger() {
    ...
    useEffect(() => {
        const handle=async()=>{
            const newconvo=await axios.get(process.Env.PORT+`api/conversation/${user?._id}`)
    ...
    useEffect(() => {
        ...
        const newuser=await axios.get(process.Env.PORT+`api/users/${receiverId}`)
    ...
    const handlemsg=async()=>{
        ...
        const res=await axios.post(process.Env.PORT+'api/messages',{
    ...
    useEffect(() => {
        const handle=async()=>{
            const newchat= await axios.get(process.Env.PORT+`api/messages/${currentchat?._id}`)
    ...
    const handleclick=async(id)=>{
        const res=await axios.get(process.Env.PORT+`api/conversation/${id}/${user._id}`)
    ...
    const convohandle=async(id)=>{
        ...
        {
           const res= await axios.post(process.Env.PORT+'api/conversation',{
                senderId: user._id,
                receiverId: id
            })
            if(res.data)
