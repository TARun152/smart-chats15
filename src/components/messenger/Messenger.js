...
export default function Messenger() {
    ...
    useEffect(() => {
        const handle=async()=>{
            const newconvo=await axios.get(`http://localhost:5000/api/conversation/${user?._id}`)
    ...
    useEffect(() => {
        ...
        const newuser=await axios.get(`http://localhost:5000/api/users/${receiverId}`)
    ...
    useEffect(() => {
        const handle=async()=>{
            const newchat= await axios.get(`http://localhost:5000/api/messages/${currentchat?._id}`)
    ...
    const handleclick=async(id)=>{
        const res=await axios.get(`http://localhost:5000/api/conversation/${id}/${user._id}`)
